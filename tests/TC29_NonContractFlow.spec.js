 ...
 * Same chain as TC13_FInalizeBidWithUIFlow.spec.js, with two differences:
 *   1. The job is created with Financial Type = "Non Contract" (via the new
 *      ProjectPage.fillNonContractJobForm() helper) instead of "Contract".
 *   2. Automation stops at contract finalization — the Invoice/OOO-approval
 *      follow-on tests from TC13 are not part of this spec.
...
test.describe.serial('Finalize contract UI flow (Non-Contract job)', () => {
    test('TC450 @regression @contract @finalizeBidUi @property @projectAndJob @nonContract : Verify E2E flow to finalize contract for a Non-Contract job', async ({
        page,
    }) => {
        ...
        Logger.step('TC450: Create property + persist propertyData (TC14 core)');
        ...
        Logger.step('TC450: Budget revision + create project (TC31)');
        ...
        Logger.step('TC450: Add Non-Contract job + contract overview edit + lastCreatedJob (TC37 trimmed)');
        await projectPage.navigateToProjects();
        ...
        await projectPage.fillNonContractJobForm({
            title: jobTitle,
            ...
            JSON.stringify(
                {
                    ...
                    createdVia: 'TC450_NonContractFlow',
        ...
        Logger.step('TC450 Scenario 2: Table → Manage Columns → hide Cost Item → assert hidden → restore');
        ...
        Logger.success('TC450 Scenario 2: Cost Item column is hidden ✓');
        await manageColumnsDialog.getByText('Cost Item', { exact: true }).click();
        await page.waitForTimeout(1000);
        ...
        Logger.success('TC450 Scenario 2: Cost Item column restored ✓');

        Logger.step('TC450: Jobs menu contract grid + finalize (TC47_NEW_UI)');
        await projectJob.runTc47NewUiContractFinalize(projectData);

        Logger.success('TC450: Full chain completed for Non-Contract job');
    });
});
